...
const LoginPage = () => {
  ...
  const continueToPoll = async () => {
    if (selectedRole === "teacher") {
      try {
        // Try to connect to backend, fallback to mock if not available
        let teacherlogin;
        try {
          teacherlogin = await axios.post(`${apiUrl}/teacher-login`);
        } catch (backendError) {
          console.warn("Backend not available, using mock login");
          // Mock response for development
          teacherlogin = {
            data: {
              username: "teacher_" + Math.random().toString(36).substr(2, 9)
            }
          };
        }
        ...
        alert("Teacher login failed. Please check if the backend server is running.");
      }
